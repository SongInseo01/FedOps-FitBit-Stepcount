...
def load_and_prepare(csv_path: str, window_size: int, batch_size):
    ...
    df = df.dropna(subset=["ActivityHour", "StepTotal"]).reset_index(drop=True)

    # 가장 데이터가 많은 참가자 선택
    target_id = df["Id"].value_counts().idxmax()
    target = (
        df.loc[df["Id"] == target_id, ["ActivityHour", "StepTotal"]]
        ...
        .ffill()
        .dropna()
        .rename(columns={"StepTotal": "Steps"})
        .reset_index()
    )
    ...
    feature_cols = ["Steps", "sin_time", "cos_time"]
...
WINDOW_SIZE = 30
CSV_PATH = "./data/hourlySteps_merged.csv"
# CSV_PATH = "/app/code/data/hourlySteps_merged.csv" # for server
batch_size = 32
